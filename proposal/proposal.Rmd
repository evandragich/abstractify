...
simplified color palette. The number of colors in the palette can be controlled 
by the user using a sliding bar to indicate a more complex or more simple color 
palette. Further, we’ll show some simple visualizations to break down the 
frequency of colors in the modified image.  Additionally, we will return the 
outlines of the image without color using the function we've created for possible
printing purposes. We have decided to accomplish these goals within a shiny app 
in order to make the application user friendly and accessible to non-R users, 
for example.
